    ...
    for i, device in enumerate(devices):
        if device['max_input_channels'] > 0:
            if required_device_name in device['name']:
                target_device_idx = i
                logger.info(f"✅ 找到指定设备: {device['name']} (Index: {i})")
                break
    
    # 🆕 未找到则启动失败
    if target_device_idx is None:
        logger.error(f"❌ 错误: 未找到音频设备 '{required_device_name}'")
        logger.info("可用设备列表:")
        for i, device in enumerate(devices):
            if device['max_input_channels'] > 0:
                logger.info(f"  [{i}] {device['name']} (输入通道: {device['max_input_channels']})")
        logger.error(f"请确保 '{required_device_name}' 已连接并被系统识别")
...
def get_audio_config(device_idx, target_sample_rate=16000, chunk_duration=0.1):
    ...
    if device_default_rate != target_sample_rate:
        logger.info(f"[INFO] 设备默认采样率 ({device_default_rate}Hz) 与模型需求 ({target_sample_rate}Hz) 不一致。")
        logger.info("尝试使用设备默认采样率进行录制并重采样...")
    ...
    if use_resample:
        try:
            import soxr
            logger.info("[INFO] 使用 soxr 进行高质量重采样")
            resampler = soxr.ResampleStream(stream_sample_rate, target_sample_rate, 1, dtype="float32")
        except ImportError:
            logger.warning("[WARN] 未找到 soxr 库，将使用 scipy.signal.resample (性能可能较低)")
            pass
